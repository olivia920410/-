...
####### 函數定義: 繪製K線圖加上RSI以及下單點位
def ChartOrder_RSI(df, trade_record): # 注意：這裡的 df 應該包含 'RSI' 欄位
   

    # 準備買入/賣出點位數據 (使用字典格式的 trade_record)
    buy_entry_times = []
    buy_entry_prices = []
    buy_cover_times = []
    buy_cover_prices = []
    ...
    for trade in trade_record:
        if isinstance(trade, dict):
            ...
            trade_type = trade.get('TradeType') # 假設您有這個欄位來區分買賣

            if trade_type == 'Buy': # 開多
                buy_entry_times.append(entry_time)
                buy_entry_prices.append(entry_price)
                buy_cover_times.append(exit_time)
                buy_cover_prices.append(exit_price)
            elif trade_type == 'Sell': # 開空
                sell_entry_times.append(entry_time)
                sell_entry_prices.append(entry_price)
                sell_cover_times.append(exit_time)
                sell_cover_prices.append(exit_price)
        # 如果 trade_record 元素是元組，您需要以下邏輯 (根據您 ChartOrder_MA 的推斷)
        # elif isinstance(trade, (list, tuple)):
        #     if trade[0] == 'Buy' or trade[0] == 'B':
        #         buy_entry_times.append(trade[2]) # EntryTime
        #         buy_entry_prices.append(trade[3]) # EntryPrice
        #         buy_cover_times.append(trade[4]) # ExitTime
        #         buy_cover_prices.append(trade[5]) # ExitPrice
        #     elif trade[0] == 'Sell' or trade[0] == 'S':
        #         sell_entry_times.append(trade[2]) # EntryTime
        #         sell_entry_prices.append(trade[3]) # EntryPrice
        #         sell_cover_times.append(trade[4]) # ExitTime
        #         sell_cover_prices.append(trade[5]) # ExitPrice


    # 創建兩個子圖：一個用於 K 線和交易點，另一個用於 RSI 指標
    fig = make_subplots(rows=2, cols=1, shared_xaxes=True, vertical_spacing=0.05,
                        row_heights=[0.7, 0.3]) # K線佔70%，RSI佔30%
    ...
    # 交易點位標記
    # 作多進場點 (綠色向上三角形)
    fig.add_trace(go.Scatter(x=buy_entry_times, y=buy_entry_prices,
                             mode='markers',
                             marker=dict(symbol='triangle-up', size=10, color='green'),
                             name='買入進場'),
                  row=1, col=1)
    # 作多出場點 (紅色向下三角形)
    fig.add_trace(go.Scatter(x=buy_cover_times, y=buy_cover_prices,
                             mode='markers',
                             marker=dict(symbol='triangle-down', size=10, color='red'),
                             name='買入出場'),
                  row=1, col=1)
    # 作空進場點 (紅色向下三角形)
    fig.add_trace(go.Scatter(x=sell_entry_times, y=sell_entry_prices,
                             mode='markers',
                             marker=dict(symbol='triangle-down', size=10, color='red'), # 與買入出場相同，或改為其他顏色區分
                             name='賣出進場'),
                  row=1, col=1)
    # 作空出場點 (綠色向上三角形)
    fig.add_trace(go.Scatter(x=sell_cover_times, y=sell_cover_prices,
                             mode='markers',
                             marker=dict(symbol='triangle-up', size=10, color='green'), # 與買入進場相同，或改為其他顏色區分
                             name='賣出出場'),
                  row=1, col=1)

    fig.update_layout(
        xaxis_rangeslider_visible=False, # 隱藏 K 線圖下方的範圍選擇器
        title_text='RSI 策略 K 線與下單點位',
        xaxis_title='時間',
        yaxis_title='價格',
    )
    ...
    # 第二個子圖：RSI 指標圖
    # 查找 RSI 欄位中第一個非 NaN 值的位置
    first_valid_rsi_index = df['RSI'].first_valid_index()
    if first_valid_rsi_index is None: # 如果 RSI 欄位全部是 NaN
        st.write("RSI 數據全部為空，無法繪製RSI線。")
        # 可以選擇在第二個子圖顯示空內容或移除第二個子圖
        # fig.update_yaxes(title_text="RSI (無數據)", row=2, col=1)
    else:
        # 確保從第一個有效 RSI 值開始繪圖
        start_index = df.index.get_loc(first_valid_rsi_index)
        fig.add_trace(go.Scatter(x=df['time'][start_index:], y=df['RSI'][start_index:],
                                 mode='lines', line=dict(color='blue', width=2), name='RSI'),
                      row=2, col=1)
        fig.add_trace(go.Scatter(x=df['time'][start_index:], y=[70]*len(df['time'][start_index:]),
                                 mode='lines', line=dict(color='red', dash='dash', width=1), name='超買 (70)'),
                      row=2, col=1)
        fig.add_trace(go.Scatter(x=df['time'][start_index:], y=[30]*len(df['time'][start_index:]),
                                 mode='lines', line=dict(color='green', dash='dash', width=1), name='超賣 (30)'),
                      row=2, col=1)
        fig.update_yaxes(title_text="RSI", range=[0, 100], row=2, col=1) # 通常RSI範圍在0-100
    ...
    # 如果 KBar_df['time'] 是 datetime 對象，可以自動設置範圍選擇器
    # fig.update_xaxes(
    #     rangeslider_visible=True, # 可以選擇再次打開 K 線範圍選擇器
    #     rangeselector=dict(
    #         buttons=list([
...
####### 函數定義: 繪製K線圖加上Bollinger Bands以及下單點位
def ChartOrder_Bollinger(df, trade_record): # 注意：這裡的 df 應該包含 'UpperBand', 'MiddleBand', 'LowerBand' 欄位


    # 準備買入/賣出點位數據 (使用字典格式的 trade_record) - 與 ChartOrder_RSI 相同
    buy_entry_times = []
    buy_entry_prices = []
    buy_cover_times = []
    buy_cover_prices = []
    ...
    # 創建圖表
    fig = make_subplots(specs=[[{"secondary_y": False}]]) # Bollinger 通常不分多個子圖
    ...
    # 添加布林通道
    # 查找第一個非 NaN 值的位置，從這裡開始繪製布林帶
    first_valid_band_index = df['MiddleBand'].first_valid_index()
    if first_valid_band_index is None:
        st.write("布林通道數據全部為空，無法繪製布林帶。")
    else:
        start_index = df.index.get_loc(first_valid_band_index)
        fig.add_trace(go.Scatter(x=df['time'][start_index:], y=df['UpperBand'][start_index:],
                                 mode='lines', line=dict(color='purple', width=1), name='上軌'),
                      secondary_y=False)
        fig.add_trace(go.Scatter(x=df['time'][start_index:], y=df['MiddleBand'][start_index:],
                                 mode='lines', line=dict(color='orange', width=2), name='中軌'),
                      secondary_y=False)
        fig.add_trace(go.Scatter(x=df['time'][start_index:], y=df['LowerBand'][start_index:],
                                 mode='lines', line=dict(color='blue', width=1), name='下軌'),
                      secondary_y=False)


    # 交易點位標記 (與 ChartOrder_RSI 相同)
    fig.add_trace(go.Scatter(x=buy_entry_times, y=buy_entry_prices,
                             mode='markers',
                             marker=dict(symbol='triangle-up', size=10, color='green'),
                             name='買入進場'))
...
if choice_strategy == choices_strategies[1]:
    ...
    ChartOrder_RSI(KBar_df, OrderRecord_RSI.GetTradeRecord())  # 臨時使用MA的繪圖函數
...
if choice_strategy == choices_strategies[2]:
    ...
    ChartOrder_Bollinger(KBar_df, OrderRecord_BB.GetTradeRecord()) # ChartOrder_MA 已更新可繪製布林通道
...
def 計算績效_期貨(order_record_obj): # 函數參數是 order_record_obj
    ...
    平均每次盈虧 =order_record_obj.GetTotalProfit() * 2     ## 取得交易 "平均" 盈虧(每次)
...
active_order_record_for_performance = None 
if active_order_record_for_performance is not None:

    if choice ==choices[0]:# '中鋼期貨: 2023.4.17 至 2025.4.17':
        交易總盈虧,平均每次盈虧,平均投資報酬率,平均獲利_只看獲利的,平均虧損_只看虧損的,勝率,最大連續虧損,最大盈虧回落_MDD,報酬風險比 = 計算績效_期貨(active_order_record_for_performance)
    # 交易總盈虧 = OrderRecord.GetTotalProfit()*1000          ## 取得交易總盈虧
    # 平均每次盈虧 = OrderRecord.GetAverageProfit()*1000         ## 取得交易 "平均" 盈虧(每次)
    # 平均投資報酬率 = OrderRecord.GetAverageProfitRate()    ## 取得交易 "平均" 投資報酬率(每次)
    # 平均獲利_只看獲利的 = OrderRecord.GetAverEarn()*1000              ## 平均獲利(只看獲利的)
...
if choice ==choices[4]:# '台新金期貨: 2023.4.17 至 2025.4.17':
    交易總盈虧,平均每次盈虧,平均投資報酬率,平均獲利_只看獲利的,平均虧損_只看虧損的,勝率,最大連續虧損,最大盈虧回落_MDD,報酬風險比 = 計算績效_期貨()
if choice ==choices[5]:# '統一期貨: 2023.4.17至 2025.4.17 ':
    交易總盈虧,平均每次盈虧,平均投資報酬率,平均獲利_只看獲利的,平均虧損_只看虧損的,勝率,最大連續虧損,最大盈虧回落_MDD,報酬風險比 = 計算績效_期貨()
...
if choice == choices[7]:#'小型臺指期: 2023.4.17至 202.4.17 ':
    交易總盈虧,平均每次盈虧,平均投資報酬率,平均獲利_只看獲利的,平均虧損_只看虧損的,勝率,最大連續虧損,最大盈虧回落_MDD,報酬風險比 = 計算績效_小台指期貨()
if choice == choices[8]:#'臺股期貨: 2023.4.17至 2025.4.17 ':
    交易總盈虧,平均每次盈虧,平均投資報酬率,平均獲利_只看獲利的,平均虧損_只看虧損的,勝率,最大連續虧損,最大盈虧回落_MDD,報酬風險比 = 計算績效_大台指期貨()
if choice == choices[9]:#'元大台灣50: 2023.4.17至 2025.4.17 ':
    交易總盈虧,平均每次盈虧,平均投資報酬率,平均獲利_只看獲利的,平均虧損_只看虧損的,勝率,最大連續虧損,最大盈虧回落_MDD,報酬風險比 = 計算績效_股票()
if choice == choices[10]:#'元大台灣50正2: 2023.4.17至 2025.4.17 ':
    交易總盈虧,平均每次盈虧,平均投資報酬率,平均獲利_只看獲利的,平均虧損_只看虧損的,勝率,最大連續虧損,最大盈虧回落_MDD,報酬風險比 = 計算績效_股票()
if choice ==choices[11]:# '台積電: 2023.4.17至 2025.4.17 ':
    交易總盈虧,平均每次盈虧,平均投資報酬率,平均獲利_只看獲利的,平均虧損_只看虧損的,勝率,最大連續虧損,最大盈虧回落_MDD,報酬風險比 = 計算績效_股票()
...
if choice ==choices[4]  : ##'台新金期貨: 2023.4.17 至 2025.4.17':
    OrderRecord.GeneratorProfitChart(choice='future4',StrategyName='MA')
if choice ==choices[5]  : ##'統一期貨: 2023.4.17 至 2025.4.17':
    OrderRecord.GeneratorProfitChart(choice='future5',StrategyName='MA')
if choice ==choices[6]  : ##'金融期貨: 2023.4.17 至 2025.4.17':
    OrderRecord.GeneratorProfitChart(choice='future6',StrategyName='MA')
if choice ==choices[7]  : ##'小型臺指期: 2023.4.17 至 2025.4.17':
    OrderRecord.GeneratorProfitChart(choice='future7',StrategyName='MA')
if choice ==choices[8]  : ##'臺股期貨: 2023.4.17 至 2025.4.17':
    OrderRecord.GeneratorProfitChart(choice='future8',StrategyName='MA')
...
if 'choice' in locals() or 'choice' in globals():
    # 針對不同的 choice 值呼叫 GeneratorProfitChart
    if choice == choices[0] :     ##'台積電: 2022.1.1 至 2024.4.9' 或 '中鋼期貨' 等
        active_order_record_for_performance.GeneratorProfitChart(choice='stock',StrategyName='MA') # <-- 這裡使用 active_order_record_for_performance
    elif choice == choices[1] :                 ##'大台指期貨2024.12到期: 2023.12 至 2024.4.11' 或 '聯電期貨' 等
        active_order_record_for_performance.GeneratorProfitChart(choice='future1',StrategyName='MA') # <-- 這裡使用 active_order_record_for_performance
    elif choice == choices[2] : # 假設這是您 for 'future0' 的地方
        active_order_record_for_performance.GeneratorProfitChart(choice='future0',StrategyName='RSI') # <<--- 這裡修正為 active_order_record_for_performance
    # 繼續添加其他 choice 的 elif 條件...
    # active_order_record_for_performance.GeneratorProfitChart(...)
else:
    st.write("請選擇一個商品以繪製盈虧圖。")
    
if choice ==choices[0]  :  ##'中鋼期貨: 2023.4.17 至 2025.4.17'
    OrderRecord_RSI.GeneratorProfitChart(choice='future0',StrategyName='RSI')
if choice ==choices[1]  : ##'聯電期貨: 2023.4.17 至 2025.4.17':
    OrderRecord_RSI.GeneratorProfitChart(choice='future1',StrategyName='RSI')
if choice == choices[2]  :##'台積電期貨: 2023.4.17 至 2025.4.17':
    OrderRecord_RSI.GeneratorProfitChart(choice='future2',StrategyName='RSI')
if choice ==choices[3]  : ##'富邦金期貨: 2023.4.17 至 2025.4.17':
    OrderRecord_RSI.GeneratorProfitChart(choice='future3',StrategyName='RSI')
if choice ==choices[4]  : ##'台新金期貨: 2023.4.17 至 2025.4.17':
    OrderRecord_RSI.GeneratorProfitChart(choice='future4',StrategyName='RSI')
if choice ==choices[5]  : ##'統一期貨: 2023.4.17 至 2025.4.17':
    OrderRecord_RSI.GeneratorProfitChart(choice='future5',StrategyName='RSI')
if choice ==choices[6]  : ##'金融期貨: 2023.4.17 至 2025.4.17':
    OrderRecord_RSI.GeneratorProfitChart(choice='future6',StrategyName='RSI')
if choice ==choices[7]  : ##'小型臺指期: 2023.4.17 至 2025.4.17':
    OrderRecord_RSI.GeneratorProfitChart(choice='future7',StrategyName='RSI')
if choice ==choices[8]  : ##'臺股期貨: 2023.4.17 至 2025.4.17':
    OrderRecord_RSI.GeneratorProfitChart(choice='future8',StrategyName='RSI')
if choice ==choices[9]  : ##'元大台灣50: 2023.4.17 至 2025.4.17':
    OrderRecord_RSI.GeneratorProfitChart(choice='stock',StrategyName='RSI')
if choice ==choices[10]  :## '元大台灣50正2: 2023.4.17 至 2025.4.17':
    OrderRecord_RSI.GeneratorProfitChart(choice='stock',StrategyName='RSI')
if choice ==choices[11]  : ##'台積電: 2023.4.17 至 2025.4.17':
    OrderRecord_RSI.GeneratorProfitChart(choice='stock',StrategyName='RSI')
...
if choice ==choices[0]  :  ##'中鋼期貨: 2023.4.17 至 2025.4.17'
    OrderRecord_BB.GeneratorProfitChart(choice='future0',StrategyName='Bollinger')
if choice ==choices[1]  : ##'聯電期貨: 2023.4.17 至 2025.4.17':
    OrderRecord_BB.GeneratorProfitChart(choice='future1',StrategyName='Bollinger')
if choice == choices[2]  :##'台積電期貨: 2023.4.17 至 2025.4.17':
    OrderRecord_BB.GeneratorProfitChart(choice='future2',StrategyName='Bollinger')
if choice ==choices[3]  : ##'富邦金期貨: 2023.4.17 至 2025.4.17':
    OrderRecord_BB.GeneratorProfitChart(choice='future3',StrategyName='Bollinger')
if choice ==choices[4]  : ##'台新金期貨: 2023.4.17 至 2025.4.17':
    OrderRecord_BB.GeneratorProfitChart(choice='future4',StrategyName='Bollinger')
if choice ==choices[5]  : ##'統一期貨: 2023.4.17 至 2025.4.17':
    OrderRecord_BB.GeneratorProfitChart(choice='future5',StrategyName='Bollinger')
if choice ==choices[6]  : ##'金融期貨: 2023.4.17 至 2025.4.17':
    OrderRecord_BB.GeneratorProfitChart(choice='future6',StrategyName='Bollinger')
if choice ==choices[7]  : ##'小型臺指期: 2023.4.17 至 2025.4.17':
    OrderRecord_BB.GeneratorProfitChart(choice='future7',StrategyName='Bollinger')
if choice ==choices[8]  : ##'臺股期貨: 2023.4.17 至 2025.4.17':
    OrderRecord_BB.GeneratorProfitChart(choice='future8',StrategyName='Bollinger')
if choice ==choices[9]  : ##'元大台灣50: 2023.4.17 至 2025.4.17':
    OrderRecord_BB.GeneratorProfitChart(choice='stock',StrategyName='Bollinger')
if choice ==choices[10]  :## '元大台灣50正2: 2023.4.17 至 2025.4.17':
    OrderRecord_BB.GeneratorProfitChart(choice='stock',StrategyName='Bollinger')
if choice ==choices[11]  : ##'台積電: 2023.4.17 至 2025.4.17':
    OrderRecord_BB.GeneratorProfitChart(choice='stock',StrategyName='Bollinger')
...
current_strategy_name = choice_strategy if choice_strategy else 'Unknown'
active_order_record_for_performance.GeneratorProfit_rateChart(StrategyName=current_strategy_name) # <-- 這裡也使用 active_order_record_for_performance
OrderRecord_RSI.GeneratorProfit_rateChart(StrategyName='RSI')
OrderRecord_BB.GeneratorProfit_rateChart(StrategyName='Bollinger')
