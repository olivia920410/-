...
if choice == '中鋼期貨: 2023.4.17 至 2025.4.17':
    start_date_str = st.text_input('輸入開始日期(日期格式: 2023-4-17), 區間:2023-4-17 至 2025-4-17', '2023-4-17')
    end_date_str = st.text_input('輸入結束日期 (日期格式: 2025-4-17), 區間:2023-4-17 至 2025-4-17', '2025-4-17')
if choice == '聯電期貨: 2023.4.17 至 2025.4.17':
   start_date_str = st.text_input('輸入開始日期(日期格式: 2023-4-17), 區間:2023-4-17 至 2025-4-17', '2023-4-17')
   end_date_str = st.text_input('輸入結束日期 (日期格式: 2025-4-17), 區間:2023-4-17 至 2025-4-17', '2025-4-17')
if choice == '台積電期貨: 2023.4.17 至 2025.4.17':
...
try:
    start_date_dt = datetime.datetime.strptime(start_date_str, '%Y-%m-%d') # 這裡使用 %Y-%m-%d
    end_date_dt = datetime.datetime.strptime(end_date_str, '%Y-%m-%d')     # 這裡使用 %Y-%m-%d
except ValueError as e:
    st.error(f"日期格式不符！請輸入正確格式的日期 (例如 2023-04-17)。錯誤詳情: {e}")
    st.stop()
    # 3. 現在，`start_date_dt` 是一個 `datetime.datetime` 物件。
#    你可以在需要的地方使用它。
#    如果你之前程式碼中篩選 DataFrame 使用的是 `start_date` 和 `end_date`，
#    請確保它們指向轉換後的 datetime 物件：
start_date = start_date_dt # 將轉換後的 datetime 物件賦值給 `start_date`
end_date = end_date_dt     # 將轉換後的 datetime 物件賦值給 `end_date`
    
# start_date = datetime.datetime.strptime(start_date_str,'%Y-%-m-%-d')
# end_date = datetime.datetime.strptime(end_date_str,'%Y-%-m-%-d')
# 使用条件筛选选择时间区间的数据
df = df_original[(df_original['time'] >= start_date_str) & (df_original['time'] <=  end_date_str)]
...
def Change_Cycle(Date,cycle_duration,KBar_dic):
    ...
    for i in range(KBar_dic['time'].size):
        ...
        # amount 參數在 AddPrice 中未使用，所以可以不傳遞，或在 AddPrice 中接收但忽略
       # amount = KBar_dic_input['amount'][i] # 根據您 AddPrice 的定義，這個參數可能不需要

       # 調用 KBar.AddPrice，確保傳入的 time 類型符合 AddPrice 的預期
        # 原始程式碼
...
# 這裡不需要將 start_date 轉換為字串，直接使用 datetime 物件
# 確保 start_date 在此之前已經是 datetime 物件
# (例如：如果它是從 st.text_input 讀入，請用 pd.to_datetime 或 datetime.datetime.strptime 轉換好)
Date = start_date # 直接使用 datetime 物件
...
# 現在 Date 變數是一個 datetime 物件，可以直接傳遞
