...
if choice == '中鋼期貨: 2023.4.17 至 2025.4.17':
    raw_start_date_str = st.text_input('輸入開始日期(日期格式: 2023-4-17), 區間:2023-4-17 至 2025-4-17', '2023-4-17')
    raw_end_date_str = st.text_input('輸入結束日期 (日期格式: 2025-4-17), 區間:2023-4-17 至 2025-4-17', '2025-4-17')
if choice == '聯電期貨: 2023.4.17 至 2025.4.17':
   raw_start_date_str = st.text_input('輸入開始日期(日期格式: 2023-4-17), 區間:2023-4-17 至 2025-4-17', '2023-4-17')
   raw_end_date_str = st.text_input('輸入結束日期 (日期格式: 2025-4-17), 區間:2023-4-17 至 2025-4-17', '2025-4-17')
if choice == '台積電期貨: 2023.4.17 至 2025.4.17':
...
try:
    # 這裡的 `%Y-%-m-%-d` 是為了匹配 '2023-4-17' 這種不補零的格式
    # 轉換後的 datetime 物件應該賦值給一個新的變數，例如 `start_date_dt`
    start_date_dt = datetime.datetime.strptime(raw_start_date_str, '%Y-%-m-%-d')
    end_date_dt = datetime.datetime.strptime(raw_end_date_str, '%Y-%-m-%-d')
except ValueError as e:
    st.error(f"日期格式不符！請輸入正確格式的日期 (例如 2023-4-17)。錯誤詳情: {e}")
    st.stop() # 如果格式錯誤，停止應用運行，避免後續錯誤   
...
# 使用条件筛选选择时间区间的数据
df = df_original[(df_original['time'] >= raw_start_date_str) & (df_original['time'] <=  raw_end_date_str)]
