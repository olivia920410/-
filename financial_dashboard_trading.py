...
# 1. 從 st.date_input 獲取日期物件
#    這會直接返回 datetime.date 物件
selected_start_date = st.date_input('選擇開始日期', value=datetime.date(2023, 4, 17))
selected_end_date = st.date_input('選擇結束日期', value=datetime.date(2025, 4, 17))

# 2. 將 datetime.date 物件轉換為 datetime.datetime 物件 (通常用於 DataFrame 篩選，因為 KBar 數據可能帶時間)
#    這裡的 `start_date_for_filter` 變數就是 datetime.datetime 物件了
start_date_for_filter = datetime.datetime.combine(selected_start_date, datetime.time.min)
end_date_for_filter = datetime.datetime.combine(selected_end_date, datetime.time.max)

# 3. 如果你的程式碼中篩選 DataFrame 還是使用 `start_date` 和 `end_date` 這些變數名，
#    請確保它們現在指向正確的 `datetime.datetime` 物件：
start_date = start_date_for_filter
end_date = end_date_for_filter

# 4. 現在，當你執行 strftime 時，你可以選擇使用 `selected_start_date` (datetime.date) 或 `start_date` (datetime.datetime)
#    兩者都有 strftime 方法，不會報錯。
Date = start_date.strftime("%Y-%m-%d")  ## 變成字串 (這裡 start_date 是 datetime.datetime)
# 或者
# Date = selected_start_date.strftime("%Y-%m-%d") ## 變成字串 (這裡 selected_start_date 是 datetime.date)
