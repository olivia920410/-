...
if choice == '中鋼期貨: 2023.4.17 至 2025.4.17':
    df_original = load_data('kbars_1d_CBF_2023-04-17_To_2025-04-17.pkl')
...
if choice == '聯電期貨: 2023.4.17 至 2025.4.17':
    df_original = load_data('kbars_1d_CCF_2023-04-17_To_2025-04-17')
    product_name = '聯電期貨CCF'
if choice == '台積電期貨: 2023.4.17 至 2025.4.17':
    df_original = load_data('kbars_1d_CDF_2023-04-17_To_2025-04-17.pkl')
    product_name = '台積電期貨CDF'
if choice == '富邦金期貨: 2023.4.17 至 2025.4.17':
    df_original = load_data('kbars_1d_CEF_2023-04-17_To_2025-04-17.pkl')
    product_name = '富邦金期貨CEF'
if choice == '台新金期貨: 2023.4.17 至 2025.4.17':
    df_original = load_data('kbars_1d_CMF_2023-04-17_To_2025-04-17.pkl')
    product_name = '台新金期貨CMF'
if choice == '統一期貨: 2023.4.17 至 2025.4.17':
    df_original = load_data('kbars_1d_CQF_2023-04-17_To_2025-04-17.pkl')
    product_name = '統一期貨CQF'
if choice == '金融期貨: 2023.4.17 至 2025.4.17':
    df_original = load_data('kbars_1d_FXF_2023-04-17_To_2025-04-17.pkl')
    product_name = '金融期貨FXF'
if choice == '小型臺指期: 2023.4.17 至 2025.4.17':
    df_original = load_data('kbars_1d_MXF_2023-04-17_To_2025-04-17.pkl')
    product_name = '小型臺指期MXF'
if choice == '臺股期貨: 2023.4.17 至 2025.4.17':
    df_original = load_data('kbars_1d_TXF_2023-04-17_To_2025-04-17.pkl')
    product_name = '臺股期貨TXF'
if choice == '元大台灣50: 2023.4.17 至 2025.4.17':
    df_original = load_data('kbars_1d_0050_2023-04-17_To_2025-04-17.pkl')
    product_name = '元大台灣50 0050'
if choice == '元大台灣50正2: 2023.4.17 至 2025.4.17':
    df_original = load_data('kbars_1d_00631L_2023-04-17_To_2025-04-17.pkl')
    product_name = '元大台灣50正2 00631L'
if choice == '台積電: 2023.4.17 至 2025.4.17':
    df_original = load_data('kbars_1d_2330_2023-04-17_To_2025-04-17.pkl')
    product_name = '台積電2330'
if choice == '華碩: 2023.4.17 至 2025.4.17':
    df_original = load_data('kbars_1d_2357_2023-04-17_To_2025-04-17.pkl')
    product_name = '華碩2357'
...
if choice == '中鋼期貨: 2023.4.17 至 2025.4.17':
    start_date = st.text_input('輸入開始日期(日期格式: 2023-4-17), 區間:2023-4-17 至 2025-4-17', '2023-4-17')
    end_date = st.text_input('輸入結束日期 (日期格式: 2025-4-17), 區間:2023-4-17 至 2025-4-17', '2025-4-17')
if choice == '聯電期貨: 2023.4.17 至 2025.4.17':
    start_date = st.text_input('輸入開始日期(日期格式: 2023-4-17), 區間:2023-4-17 至 2025-4-17', '2023-4-17')
    end_date = st.text_input('輸入結束日期 (日期格式: 2025-4-17), 區間:2023-4-17 至 2025-4-17', '2025-4-17')
if choice == '台積電期貨: 2023.4.17 至 2025.4.17':
...
def Change_Cycle(initial_datetime_obj, cycle_duration, KBar_dic_input, product_name):
    # KBar 物件初始化時，傳入 datetime 物件作為起始日期
    KBar = indicator_forKBar_short.KBar(initial_datetime_obj, cycle_duration)
    
    # 遍歷原始 KBar_dic 的數據
    for i in range(KBar_dic_input['time'].size):
        # 確保 KBar_dic_input['time'][i] 是 datetime.datetime 物件
        # 這裡可以再次進行診斷，如果它不是 datetime.datetime，就說明 To_Dictionary_1 有問題
        current_time_val = KBar_dic_input['time'][i]
        # st.sidebar.write(f"In Change_Cycle loop, time_val type: {type(current_time_val)}, value: {current_time_val}") # Debugging line

        # 確保 AddPrice 接收的是正確的類型
        # 如果 `current_time_val` 仍然是 `numpy.datetime64`，Pandas 內部可能會自動轉換
        # 或者直接確保它轉成 Python 的 datetime.datetime 物件
        if isinstance(current_time_val, np.datetime64):
            current_time_val = pd.to_datetime(current_time_val).to_pydatetime()
        elif not isinstance(current_time_val, datetime.datetime):
            # 如果不是 datetime.datetime 也不是 np.datetime64，則可能是錯誤類型
            st.error(f"錯誤：KBar_dic_input['time'][{i}] 的類型是 {type(current_time_val)}，預期為 datetime.datetime。值：{current_time_val}")
            st.stop()


        open_price= KBar_dic_input['open'][i]
        close_price= KBar_dic_input['close'][i]
        low_price= KBar_dic_input['low'][i]
        high_price= KBar_dic_input['high'][i]
        qty = KBar_dic_input['volume'][i] # 確保這個鍵名正確，如果數據中有 'volume'

        tag=KBar.AddPrice(current_time_val, open_price, close_price, low_price, high_price, qty)
    
    # 形成 KBar 字典 (新週期的): 從 KBar 物件的 TAKBar 屬性獲取數據
    # 確保 TAKBar 內的 time 也是 datetime.datetime 物件或可轉換為 NumPy 陣列
    new_KBar_dic = {}
    new_KBar_dic['time'] = KBar.TAKBar['time']
    new_KBar_dic['open'] = KBar.TAKBar['open']
    new_KBar_dic['high'] = KBar.TAKBar['high']
    new_KBar_dic['low'] = KBar.TAKBar['low']
    new_KBar_dic['close'] = KBar.TAKBar['close']
    new_KBar_dic['volume'] = KBar.TAKBar['volume']
    
    # product 欄位：確保與新 K 棒數量一致
    new_KBar_dic['product'] = np.repeat(product_name, new_KBar_dic['time'].size)
    
    return new_KBar_dic

# 呼叫 Change_Cycle 函數
# 傳遞 `start_date` (這是 datetime.datetime 物件)，而不是字串 `Date`
KBar_dic = Change_Cycle(start_date, cycle_duration, KBar_dic, product_name)
