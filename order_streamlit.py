...
class Record():
    ...
    def GeneratorProfitChart(self, choice='stock', StrategyName='Strategy'):
        ...
        fig = plt.figure(figsize=(10, 6)) # 創建一個圖形對象並賦值給變數 `fig`
        ax = fig.add_subplot(111) # 創建一個子圖，並賦值給 `ax`

        #### 計算累計績效
        TotalProfit=[0]
        # 只有當 self.Profit 有數據時才進行累計
        if len(self.Profit) > 0:
            for i in self.Profit:
                TotalProfit.append(TotalProfit[-1] + i)

        #### 繪製圖形
        # 只有當 TotalProfit 有足夠的數據點 (至少包含一次交易的利潤) 才繪圖
        if len(TotalProfit) > 1: # TotalProfit至少會有[0, first_profit, ...]
            multiplier = 1
            if choice == 'stock':
                multiplier = 1000
            elif choice == 'future1':
                ...
                multiplier = 50
            # 添加其他期貨的乘數
            # elif choice == 'future3': multiplier = 1000 # 中鋼期貨
            # elif choice == 'future4': multiplier = 200  # 聯電期貨
            # ... 確保您的所有 choice 都被考慮進去

            plottable_profit = [i * multiplier for i in TotalProfit[1:]]

            ax.plot(plottable_profit, '-', marker='o', linewidth=1)

            #### 定義標頭
            ax.set_title('累計盈虧(元)')
            ax.set_xlabel('交易編號')
            ax.set_ylabel('累計盈虧(元)')

            #### 設置x軸的刻度
            length = len(plottable_profit) # 實際繪圖的數據點數量
            if length > 0:
                new_ticks = range(1, length + 1)
                ax.set_xticks(ticks=range(length), labels=new_ticks)

            ### 在Streamlit中显示
            st.pyplot(fig) # <<<<< 將圖形對象 `fig` 傳遞給 st.pyplot
        else:
            # 即使有這行，如果 st.write('沒有交易記錄...') 已經顯示，這裡可能就不需要再顯示了
            st.write('累計盈虧圖沒有足夠數據可繪製。') 

        plt.close(fig) # 關閉當前圖形，防止記憶體洩漏和圖形重疊
        
    ## 產出交易績效圖(累計投資報酬率)
    def GeneratorProfit_rateChart(self, StrategyName='Strategy'):
    import matplotlib.pyplot as plt # 確保這裡也導入
    import matplotlib
    ...
    #### 繪製圖形
    if len(TotalProfit_rate) > 1: # 確保有數據可繪圖
        # 這裡可能不需要乘數，因為是報酬率
